...
namespace bsn.GoldParser.Semantic {
	public class SemanticTypeActions<T>: SemanticActions<T> where T: SemanticToken {
        ...
        private delegate void GetFactoryTypeDel<TU>(TU methodBase, RuleAttribute ruleAttribute, Type type, out Type factoryType, out TU factoryConstructor) where TU : MethodBase;
			...
			foreach (Type type in typeof(T).Assembly.GetTypes()) {
				if (typeof(T).IsAssignableFrom(type) && type.IsClass && (!type.IsAbstract)) {
					...
					foreach (ConstructorInfo constructor in type.GetConstructors()) {
						...
						{
						    ProcessAttribute(errors, strongParameterCheck, ruleAttribute, type, constructor, GetFactoryType);
						}
					}
				}
			        ...
			        if(typeof(T).IsAssignableFrom(methodInfo.ReturnType))
			        {
                        foreach (RuleAttribute ruleAttribute in methodInfo.GetCustomAttributes(typeof(RuleAttribute), true))
                        {
                            if(methodInfo.IsStatic)
                                ProcessAttribute(errors, strongParameterCheck, ruleAttribute, type, methodInfo, GetFactoryType);
                            else
        ...
        //ConstructorInfo constructor, RuleAttribute ruleAttribute, Type type, out Type factoryType, out ConstructorInfo factoryConstructor
	    private static void GetFactoryType(MethodInfo methodInfo, RuleAttribute ruleAttribute, Type type, out Type factoryType,out MethodInfo factoryMethod)
	    {
	    ...
	    private void ProcessAttribute<TU>(ICollection<string> errors, bool strongParameterCheck, RuleAttribute ruleAttribute, Type type, TU methodBase, GetFactoryTypeDel<TU> getFactoryTypeDel) where TU : MethodBase
	    {
	        ...
	        else
	        {
	            try
	            {
	                ...
	                getFactoryTypeDel(methodBase, ruleAttribute, type, out factoryType, out factoryConstructor);
	                var parameterMapping = ParameterMapping(strongParameterCheck, rule, ruleAttribute, methodBase);
...
#pragma warning disable 612,618
	        ...
	        if (ruleAttribute.HasConstructorParameterMapping)
	        {
	            ...
	            if (parameterMapping == null)
	            {
	                parameterMapping =
	                    new int[
	                        ruleAttribute.AllowTruncationForConstructor ? methodBase.GetParameters().Length : rule.SymbolCount];
	        ...
	        {
	            parameterMapping = RuleDeclarationParser.BindMethodBase(ruleAttribute.ParsedRule, methodBase, ruleAttribute.AllowTruncationForConstructor,BijectiveMapping(strongParameterCheck, ruleAttribute));
	        }
	        return parameterMapping;
#pragma warning restore 612,618
        }

	    private static bool BijectiveMapping(bool strongParameterCheck, RuleAttribute ruleAttribute)
	    {
	        return ruleAttribute.StrictlyMatchCtorParameters.HasValue ? ruleAttribute.StrictlyMatchCtorParameters.Value: strongParameterCheck;
	    }

	    private static void GetFactoryType(ConstructorInfo constructor, RuleAttribute ruleAttribute, Type type, out Type factoryType, out ConstructorInfo factoryConstructor)
	    {
	        if (ruleAttribute.IsGeneric)
	        {
	            ...
	            {
	                foreach (
	                    RuleAttribute genericRuleAttribute in
	                        genericConstructor.GetCustomAttributes(typeof (RuleAttribute), true))
	                {
		...
		private SemanticNonterminalFactory<T> CreateNonterminalFactory(Type type, MethodBase methodBase, int[] parameterMapping, int handleCount) {
