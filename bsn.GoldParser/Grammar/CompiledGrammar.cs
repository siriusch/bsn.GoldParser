...
namespace bsn.GoldParser.Grammar {
	...
	public class CompiledGrammar {
		...
		/// </summary>
		[Obsolete("This property is always true with the new EGT file format")]
		...
		private void Load(LoadContext context) {
			...
			FixupGroups(context);
			blockCommentStates = (fileVersion == CgtVersion.V1_0) ? GetDfaStatesOfSymbols(symbol => (symbol.Kind == SymbolKind.BlockStart) || (symbol.Kind == SymbolKind.BlockEnd)) : new DfaState[0];
		}
		...
		private void FixupGroups(LoadContext context) {
			for (int i = 0; i < groupTable.Length; i++) {
				...
				groupTable[i].Initialize(GetSymbol(info.ContainerIndex), GetSymbol(info.StartIndex), GetSymbol(info.EndIndex), nesting);
			}
		}
